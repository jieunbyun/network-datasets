...
`data/ieee14.m` is the standard IEEE 14-bus test case, distributed with
...
the 54.8% blackout threshold (Scenario 1) follow:
...
which reports a reference failure probability of ≈ 1.1 × 10⁻⁴ for the
branches+buses variant.

## Data files in this dataset

The JSON files (`nodes*.json`, `edges*.json`, `probs*.json`) and the
DC-OPF system-function script (`sfun_dcopt.py`, `func_dcopt_py.py`,
`run_case14*.py`) were prepared by the authors of Byun et al. (2024) and
